...
void EncodeFile()
{
	...
	cout << "Escreva o nome do arquivo que deseja codificar: \n\n";
	cin >> nomeArquivo;
	ifstream fin;
	text.clear();
	char b;
	...
	lHeader.sort([](const Header& first, const Header& second) { return first.symb < second.symb; });
	...
	for (int i = 0; i < alphabetSize; i++)
	{
		...
		no->symb = header[i].symb;
		no->freq = header[i].freq;
		no->dir = nullptr;
		no->esq = nullptr;
...
void DecodeFile()
{
	...
	cout << "Escreva o nome do arquivo que deseja decodificar: \n\n";
	cin >> nomeArquivo;
	...
	for (int i = 0; i < alphabetSize; i++)
	{
		...
		hNo->esq = nullptr;
		hNo->dir = nullptr;
		hNo->symb = header[i].symb;
		hNo->freq = header[i].freq;
...
string CharToBin(unsigned char input) // Converte caractere em binario.
{
...
HeaderNo * CreateTree()
{
	...
	while (simpleFrontier.elements.size() > 1)
	{
		...
		no->esq = n1;
		no->dir = n2;
		no->freq = n1->freq + n2->freq;
		no->symb = '\0';

		simpleFrontier.put(no, n1->freq + n2->freq); // Adiciona nó à fila de prioridade
	}
...
int CreateAlphabet(char c)
{
	...
	for (int i = 0; i < alphabetSize; i++)
	{
		// Verifica se o caractere c já foi inserido.
		if (totalHeader[i]->symb == c)
		{
			totalHeader[i]->freq++;
			return -1;
		}
	...
	totalHeader[alphabetSize]->symb = c;
